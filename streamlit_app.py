...
# Interface do usuário para inserir a URL da página de pesquisa
...
if st.button('Pesquisar e Raspagem de Dados'):
    try:
        ...
        # Localize os elementos HTML com base na estrutura da página
        product_names = [element.text.strip() for element in soup.select('.product-title h1')]
        product_prices = [element.text.strip() for element in soup.select('span.d-inline-block h4#price_display.js-price-display.h2.mb-2.text-brand')]
        product_descriptions = [element.text.strip() for element in soup.select('.product-description')]
        image_urls = [element.get('src') for element in soup.select('div.js-product-slide:nth-child(1) > a:nth-child(1) > img:nth-child(1)')]
